...
def sucess():
    ...
    if(request.method == 'POST'):
        ...
        #response = f'Hi {node2}! this is Python ',
        ...
        img = cv2.imread(r'C:\Users\user\Desktop/map.jfif')  
        node2 = request_data['node2'] 
        path = graph1.a_star_algorithm(1, node2)
        #response=draw(path,img)
        response=path
...
if __name__== '__main__':
    app.run(host='192.168.1.249',port=5000,debug=True)  
